...
2. Create the two root-only Better Auth operator realms and their separate passwords, dedicated Stalwart sender identities/SMTP credentials, session, SQLite/audit, cookie, and recovery state without exposing secret values in shell arguments or logs. Attach each dashboard only to its matching internal `mail-submit` network and verify certificate-checked STARTTLS to `mail.esmii.app`.
...
4. Activate staging Prometheus and `staging-dashboard` with the immutable full-SHA dashboard digest, private staging networks/state, and no public route. Verify password-triggered email delivery, single-use/expiry behavior, and anonymous/password-only/wrong-host denial.
...
- Every monitoring page/API requires an eight-hour Better Auth operator session completed with password plus a five-minute single-use email OTP; no dashboard credential or session authorizes customer application access.
