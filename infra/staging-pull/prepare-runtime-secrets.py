...
CANONICAL_ROOT = pathlib.Path("/etc/myapp/secrets/staging")
RUNTIME_ROOT = pathlib.Path("/etc/myapp/runtime-secrets/staging")
...
def ensure_root_directory(path: pathlib.Path, *, create: bool) -> None:
    ...
    if path.is_symlink() or not path.is_dir() or information.st_uid != 0:
        raise ValueError(f"refusing unsafe staging secret directory: {path}")
    path.chmod(0o700)
...
def copy_runtime_secret(filename: str) -> None:
    ...
    if source.is_symlink() or not source.is_file() or information.st_uid != 0:
        raise ValueError(f"refusing unsafe canonical staging secret: {source}")
    source.chmod(0o600)
    contents = source.read_bytes()
    if not contents.strip():
        raise ValueError(f"refusing empty canonical staging secret: {source}")
    ...
    if destination.exists() or destination.is_symlink():
        ...
        if (
            ...
            or destination_information.st_uid != 0
        ):
            raise ValueError(f"refusing unsafe runtime staging secret: {destination}")
...
def main() -> int:
    ...
    print("Prepared container-readable staging secret mounts without printing credential values.")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"staging runtime secret preparation failed: {error}", file=sys.stderr)
