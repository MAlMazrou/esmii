...
The repository contains the **active Prompt 05 dev-to-staging path on `dev`** after the reviewed bootstrap, WireGuard/passwordless-sudo, rescue-recovery, SSH/UFW hardening, DNS, Google-only OAuth, CI publication, and staging activation gates. Prompts 02–04 retain their recorded evidence. The saved ignored inventory uses the VPN operator and hardened flags; public root/operator SSH is denied, UFW is active, management SSH is restricted to `10.77.0.2`, and the Netcup provider firewall is unchanged. A successful protected-`dev` CI run now publishes immutable candidate images; the root-owned `esmii-staging-pull.timer` polls outbound and automatically migrates and activates the exact successful revision. `https://staging.esmii.app` serves the real noindex Esmii application through the permanent Caddy, web, API, worker, PostgreSQL, Valkey, and private Mailpit staging services; the former direct-demo container is stopped. Staging credentials remain root-owned outside Git. Production remains absent (`main` is not deployed), Stalwart/internet mail is inactive, and Prompt 06 is not authorized. Do not claim sealed production promotion, production backups/mail, or public production exists until fresh evidence proves it.
