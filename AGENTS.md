...
The repository contains the **active Prompt 05 dev-to-staging path on `dev`** after the reviewed bootstrap, WireGuard/passwordless-sudo, rescue-recovery, SSH/UFW hardening, DNS, Google-only OAuth, CI publication, and staging activation gates. Prompts 02–04 retain their recorded evidence. The saved ignored inventory uses the VPN operator and hardened flags; public root/operator SSH is denied, UFW is active, management SSH is restricted to `10.77.0.2`, and the Netcup provider firewall is unchanged. A successful protected-`dev` CI run publishes immutable candidate images; the root-owned `esmii-staging-pull.timer` polls outbound and automatically migrates and activates the exact successful revision. `https://staging.esmii.app` serves the real noindex Esmii application through isolated staging services. On 30 August 2026 the user authorized Prompt 06's initial public application gate: `main` receives the equivalent CI-gated outbound pull path for a separate production runtime at `https://esmii.app`, public immediately. This gate deliberately keeps internet mail, production Google OAuth, real-user onboarding, offsite backup acceptance, and final production-hardening acceptance inactive until their own inputs exist. Do not claim those deferred capabilities merely because the public empty application shell is reachable.
