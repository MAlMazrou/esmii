...
The repository contains the **locally verified Prompt 04 implementation on `dev`**, and Prompt 05 is in progress after the reviewed bootstrap, WireGuard/passwordless-sudo, rescue-recovery, SSH/UFW hardening, DNS, and Google-only OAuth gates. Prompts 02 and 03 retain their recorded local evidence. The saved ignored inventory uses the VPN operator and hardened flags; public root/operator SSH is denied, UFW is active, management SSH is restricted to `10.77.0.2`, and the Netcup provider firewall is unchanged. Under the user's explicit 30 August 2026 instruction to make the empty demo visible without waiting for the permanent release pipeline, `https://staging.esmii.app` now serves a noindex placeholder from one digest-pinned Caddy container at `/srv/myapp/staging-demo`. It has valid automatic HTTPS, a read-only root filesystem, bounded memory/PIDs, root-owned inputs, and Docker restart persistence. This is a temporary direct first-demo exception, not the sealed Esmii application release. No staging secret or OAuth credential is installed; no PostgreSQL, Valkey, Mailpit, API, worker, application web image, reconciler, Esmii timer, registry publication, immutable checkpoint, production service, or internet mail exists. Do not claim the full Prompt 05 deployment/CI/pull path exists until fresh evidence proves it.
...
- `main` records the code tree currently verified in production. It has no direct or force pushes and is advanced only by the protected production-promotion workflow after the exact staging-tested digests pass production deployment and smoke checks.
- Production promotion never rebuilds the web or server images. It verifies the currently active healthy staging release, deploys those exact application/image digests, then advances `main` fast-forward-only to that source SHA. If production fails or `main` cannot fast-forward, roll back or stop and leave `main` unchanged. A later runtime rollback is recorded by a reviewed forward rollback/revert commit whose tree matches the restored release; never force-move `main` backward, and merge that rollback record back into `dev` before the next production promotion.
...
- Prompt 01 is read-only. Repository implementation work in Prompts 02–05 remains on `dev`. Prompt 06 is the only prompt allowed to advance `main`, and doing so still requires its explicit external-action approval.
...
The Prompt 05 ongoing staging policy is narrow: it covers only successful protected-`dev` candidates using the unchanged reviewed workflow/reconciler, signed immutable artifacts, expected predecessor, and a staging-only state transition with resource/isolation checks and tested rollback. Before production exists it preserves `production: null`; afterward it must preserve the complete production block, shared-infrastructure payload, and non-staging overlays. Any workflow, credential, policy, host, provider, secret, migration-policy, or environment-boundary change requires new approval. Every production promotion remains manual and separately approved.
